...
class Portfolio():
    def __init__(self, config: dict):
        ...
        self.worker_threads = [
            threading.Thread(target=self.reset_thread)
        ]
        ...
        if self.quotes != new_quotes:
            ...
            if not ask_px_changed:
                for ask in self.asks:
                    if float(ask['price']) == float(new_quotes['ask_px']) and float(ask['origQty']) >= float(new_quotes['ask_qty']):
                        self._logger.warning(f"We're the only one quoting on the ask side! Retreating to secondary level {new_quotes['ask_px']} -> {new_quotes['secondary_ask_px']}")
            ...
            if not bid_px_changed:
                for bid in self.bids:
                    if float(bid['price']) == float(new_quotes['bid_px']) and float(bid['origQty']) >= float(new_quotes['bid_qty']):
                        self._logger.warning(f"We're the only one quoting on the bid side! Retreating to secondary level {new_quotes['bid_px']} -> {new_quotes['secondary_bid_px']}")
            ...
            if len(orders_to_cancel) > 0:
                response = client.cancel_batch_order(
                    symbol="BTCUSDT", orderIdList=orders_to_cancel, origClientOrderIdList=orders_to_cancel, recvWindow=2000
                )
